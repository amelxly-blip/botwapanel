...
const CONFIG = {
  ownerNumber: "6282174905591",
  botName: "MCHLERN AI",
  ownerName: "MCHLERN PROJECT",
  prefix: "."
};
  ...
  <body style="background:#020617; color:#f8fafc; font-family:sans-serif; display:flex; justify-content:center; align-items:center; min-height:100vh; margin:0; padding:15px;">
    <div style="background:#0f172a; border:1px solid #1e293b; padding:24px; border-radius:16px; width:100%; max-width:460px; box-shadow:0 10px 25px rgba(0,0,0,0.5);">
      ...
      <div id="resultBox" style="display:none; background:#020617; border:1px solid rgba(52,211,153,0.3); padding:12px; border-radius:8px; text-align:center; margin-bottom:15px;">
        ...
        <h2 id="codeDisplay" style="color:#34d399; font-family:monospace; margin:0; font-size:26px; letter-spacing:2px;">Memuat...</h2>
      </div>
    ...
    <script>
      ...
      function requestPairing() {
        ...
        document.getElementById("codeDisplay").innerText = "Meminta kode...";
        fetch("/pairing", { method: "POST", headers: {"Content-Type": "application/json"}, body: JSON.stringify({ phone }) });
      }
      ...
      socket.on("bot-connected", () => {
        alert("Bot berhasil terhubung! Halaman akan dimuat ulang.");
        location.reload();
      });
    </script>
  </body>
...
app.post("/pairing", async (req, res) => {
  ...
  try {
    ...
    sock.ev.on("connection.update", (update) => {
      ...
      if (connection === "open") io.emit("bot-connected");
